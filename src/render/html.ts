...
export function renderHtml(source: string, _title: string, _config: Config): RenderedArtifact {
  ...
  const warnings = [];
  if (
    /<(?:script|link|img|iframe|video|audio)\b[^>]*\b(?:src|href)\s*=\s*["']?(?!data:|#)/i.test(
      source,
    )
  ) {
    warnings.push({
      code: 'W_EXTERNAL_ASSETS',
      message:
        'This HTML may reference separate resources. Exhibit only publishes this file; the default CSP blocks external resources.',
    });
  }
  if (!/<html(?:\s|>)/i.test(source)) {
    warnings.push({
      code: 'W_HTML_FRAGMENT',
      message:
        'This is an HTML fragment. The browser will infer a document; a standalone HTML document is recommended.',
    });
  }
