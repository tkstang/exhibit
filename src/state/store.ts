...
function validReceipt(value: unknown): value is PublicationReceipt {
  ...
  return record.schemaVersion === 1 && typeof record.slug === 'string'
    && typeof record.url === 'string' && typeof record.savedAt === 'string'
    ...
    && typeof record.bodySha256 === 'string' && /^[a-f0-9]{64}$/.test(record.bodySha256);
}
...
export function createPublicationState(config: Config, stateRoot: string): PublicationState {
  ...
  const path = (slug: string, digest: string) => {
    if (!/^[a-f0-9]{64}$/.test(digest)) throw new ExhibitError('E_STATE', 'Invalid local publication identity.');
  ...
  return {
    async read(slug, digest) {
      ...
      try {
        ...
        if (!info.isFile() || info.isSymbolicLink() || (process.platform !== 'win32' && (info.mode & 0o077) !== 0)) {
          throw new ExhibitError('E_STATE', 'Local publication state has unsafe permissions or type.', { hint: 'Receipts must be regular files, mode 0600, in a private directory.' });
        }
      } catch (error) { if (hasCode(error, 'ENOENT')) return null; throw error; }
      let value: unknown;
      try { value = JSON.parse(await readTextFile(target, 64 * 1024)); }
      catch { throw new ExhibitError('E_STATE', 'Local publication state is unreadable or invalid.'); }
      if (!validReceipt(value) || value.slug !== slug || value.bodySha256 !== digest) {
        throw new ExhibitError('E_STATE', 'Local publication receipt does not match this artifact.');
      }
      return value;
    },
    async save(receipt) {
      if (!validReceipt(receipt)) throw new ExhibitError('E_STATE', 'Refusing an invalid publication receipt.');
    ...
    async remove(slug) {
      ...
      try {
        ...
        if (!info.isDirectory() || info.isSymbolicLink()) throw new ExhibitError('E_STATE', 'Refusing an unsafe local state directory.');
