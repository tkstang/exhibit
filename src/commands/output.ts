...
export function humanOutput(envelope: Envelope): string {
  ...
  if (envelope.command === 'remove') return `${data.dry_run ? 'Would remove' : data.removed ? 'Removed' : 'Already absent'}: ${String(data.slug)}\n`;
  ...
  if (envelope.command === 'doctor' && Array.isArray(data.checks)) {
    return data.checks.map((item: unknown) => { const check = record(item); return `${String(check.status).toUpperCase()} ${String(check.name)}: ${String(check.message)}`; }).join('\n') + '\n';
  }
...
export function diagnostics(envelope: Envelope): string {
  ...
  return data.warnings.map((item: unknown) => {
