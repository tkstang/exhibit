...
import type { ArtifactStore, Config, PublicationReceipt, PublicationState, RenderedArtifact, Warning } from '#core/types';
...
export async function publishArtifact(options: PublishOptions, dependencies: PublishDependencies) {
  if (options.public && options.password !== undefined) throw new ExhibitError('E_USAGE', '--public cannot be combined with a password.');
  if (options.overwrite && !options.slug) throw new ExhibitError('E_USAGE', '--overwrite requires an explicit --slug.');
  if (options.allowSecrets && options.strictSecrets) throw new ExhibitError('E_USAGE', '--allow-secrets and --strict-secrets cannot be combined.');
  ...
  const input = await (dependencies.read ?? readArtifact)(options.file, config.maxInputBytes, options.title);
  const findings = scanSecrets(input.text);
  const warnings: Warning[] = [];
  if (input.type === 'html' && options.title !== undefined) {
    warnings.push({ code: 'W_HTML_TITLE', message: '--title applies to Markdown. The standalone HTML document keeps its authored title.' });
  }
  if (findings.length) {
    if ((options.public || options.strictSecrets) && !options.allowSecrets) {
      throw new ExhibitError('E_SECRET_DETECTED', 'Potential secrets were found; nothing was uploaded.', {
    ...
    warnings.push({ code: 'W_SECRETS', message: `Potential secrets found (${findings.length} matches). Inspect the source before sharing.` });
  }
  const rendered = await (dependencies.render ?? renderArtifact)(input, config);
  warnings.push(...rendered.warnings);
  if (options.dryRun) {
    return { dry_run: true, slug, url, source_type: input.type, protected: !options.public,
      rendered_bytes: Buffer.byteLength(rendered.html), would_overwrite: Boolean(options.overwrite), warnings };
  }
  if (options.password !== undefined) validatePassword(options.password);
  const existing = await store.head(slug);
  if (existing?.kind === 'probe') throw new ExhibitError('E_NOT_MANAGED', 'This slug belongs to a diagnostic probe.');
  if (existing && !options.overwrite) throw new ExhibitError('E_CONFLICT', 'This slug already exists.', { hint: 'Choose another slug, or use --overwrite to replace this Exhibit artifact conditionally.' });
  ...
  const body = password === null
    ...
    : await (dependencies.protect ?? ((html, key, settings) => protectHtml(html, key, settings, createProtector())))(rendered.html, password, config);
  const date = (dependencies.now ?? (() => new Date()))().toISOString();
  const bodySha256 = sha256(body);
  const receipt: PublicationReceipt = {
    schemaVersion: 1, slug, url, password, bodySha256, etag: null, status: 'prepared', savedAt: date,
  };
  ...
  const saved = await store.put({
    slug, body, type: input.type, kind: 'artifact', protected: password !== null,
    createdAt: existing?.createdAt ?? date, updatedAt: date,
    ...(existing ? { expectedEtag: existing.etag } : {}),
  });
  let stateSaved = false;
  if (!options.noStorePassword) {
    try { await state.save({ ...receipt, etag: saved.etag, status: 'published' }); stateSaved = true; }
    catch {
      stateSaved = persistPassword;
      warnings.push({ code: 'W_STATE_SAVE', message: persistPassword
    ...
    warnings.push({ code: 'W_PASSWORD_NOT_STORED', message: 'This password is only in the publication result. Save it securely now.' });
  }
  if (options.password !== undefined) {
    warnings.push({ code: 'W_CUSTOM_PASSWORD', message: 'A length check does not measure password strength. Use a unique high-entropy password.' });
  }
  return {
    dry_run: false, id: slug, slug, url, source_type: input.type, protected: password !== null,
    password, etag: saved.etag, body_sha256: bodySha256, size_bytes: saved.size,
    created_at: saved.createdAt, updated_at: saved.updatedAt,
    overwritten: existing !== null, state_saved: stateSaved, warnings,
  };
}
