...
import type { Config, Warning } from '#core/types';
import { createPublicationState, enumerateReceipts } from '#state/store';
...
export async function inspectReceipts(
  ...
  const receipts = await enumerateReceipts(config, stateRoot, slug);
  return {
    slug,
    remote_checked: false,
