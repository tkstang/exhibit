...
describe('deployment doctor', () => {
  ...
  it('exercises encryption, public bytes, conditions, and cleanup', async () => {
    ...
    const r = await doctor(
      ...
      store,
      { probe: true },
      { fetch: mockedFetch, protector: createProtector() },
    );
    assert.equal(r.healthy, true, JSON.stringify(r));
    assert.equal(transport.objects.size, 0);
    assert.ok(r.checks.some((c) => c.name === 'conditional-delete' && c.status === 'pass'));
  });
  it('cleans up even when public retrieval fails', async () => {
    const { store, transport } = memory();
    const mockedFetch: typeof fetch = async () => new Response('not found', { status: 404 });
    const r = await doctor(config, store, { probe: true }, { fetch: mockedFetch });
    assert.equal(r.healthy, false);
    assert.equal(transport.objects.size, 0);
  });
  it('limits remote probe response size', async () => {
