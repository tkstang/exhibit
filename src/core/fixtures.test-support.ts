...
export function s3Error(name: string, status: number) {
  return { name, $metadata: { httpStatusCode: status } };
}
...
export class MemoryTransport implements S3Transport {
  ...
  async put(input: PutRequest) {
    ...
    if ((input.IfNoneMatch === '*' && old) || (input.IfMatch && old?.etag !== input.IfMatch))
  ...
  async head(input: { Bucket: string; Key: string }): Promise<HeadResponse> {
    ...
    return {
      ETag: found.etag,
      Metadata: found.request.Metadata,
      ContentLength: Buffer.byteLength(found.request.Body),
      ContentType: found.request.ContentType,
      ContentDisposition: found.request.ContentDisposition,
    };
  }
  async list(input: ListRequest): Promise<ListResponse> {
    ...
    const all = [...this.objects.keys()]
      .filter(
        (key) => key.startsWith(input.Prefix) && !key.slice(input.Prefix.length).includes('/'),
      )
      .sort();
    ...
    return {
      ...
      ...(truncated ? { NextContinuationToken: String(start + selected.length) } : {}),
    };
  }
...
export function memory() {
  ...
  const state: PublicationState = {
    async read(slug, digest) {
      return receipts.get(`${slug}/${digest}`) ?? null;
    },
    async save(receipt) {
