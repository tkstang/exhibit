...
export async function readTextFile(path: string, maximum: number): Promise<string> {
  ...
  try {
    ...
    if (!info.isFile() || info.isSymbolicLink()) throw new ExhibitError('E_INPUT', 'Expected a regular, non-symlink file.');
    handle = await open(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
    const stat = await handle.stat();
    if (!stat.isFile()) throw new ExhibitError('E_INPUT', 'Expected a regular file.');
    if (stat.size > maximum) throw new ExhibitError('E_INPUT_SIZE', 'Input exceeds the configured size limit.');
    ...
    if (total > maximum) throw new ExhibitError('E_INPUT_SIZE', 'Input exceeds the configured size limit.');
    try { return new TextDecoder('utf-8', { fatal: true }).decode(bytes.subarray(0, total)); }
    catch { throw new ExhibitError('E_INPUT_ENCODING', 'Input must be valid UTF-8 text.'); }
  } catch (error) {
...
export async function ensurePrivateDirectory(path: string): Promise<void> {
  try {
    ...
    if (process.platform !== 'win32' && (stat.mode & 0o077) !== 0) throw new Error('directory not private');
  } catch {
...
export async function writePrivateJson(path: string, value: unknown, overwrite = true, privateDirectory = true): Promise<void> {
  if (privateDirectory) await ensurePrivateDirectory(dirname(path));
  else {
    await mkdir(dirname(path), { recursive: true, mode: 0o700 });
    const parent = await lstat(dirname(path));
    if (!parent.isDirectory() || parent.isSymbolicLink()) throw new ExhibitError('E_CONFIG', 'Unsafe config directory.');
  }
  ...
  try {
    try {
      ...
      if (!old.isFile() || old.isSymbolicLink()) throw new ExhibitError('E_STATE', 'Refusing an unsafe state/config target.');
      if (!overwrite) throw new ExhibitError('E_CONFIG_EXISTS', 'Configuration already exists.', { hint: 'Choose another --config path or explicitly pass --force.' });
...
`, 'utf8');
    ...
    if (!overwrite) {
      ...
      try { await link(temporary, path); }
        ...
        if (hasCode(error, 'EEXIST')) throw new ExhibitError('E_CONFIG_EXISTS', 'Configuration already exists.');
        throw error;
      }
    ...
    if (error instanceof ExhibitError) throw error;
    throw new ExhibitError('E_STATE', 'Could not persist local configuration or publication state.', {
